...
const defaultConfig = require('../lex-js/src/config-default.js');
...
runTest("Local file selection", withPage(async (t, page, server) => {
  ...
  await fileSelect.uploadFile('./lex-js/src/app.js');
  ...
  await fileSelect.uploadFile('./lex-js/src/ui.js', './lex-js/src/scroll.js');
...
runTest("Keyboard navigation", withPage(async (t, page, server) => {
  ...
  const fileSelect = await page.$('#file-select');
  await fileSelect.uploadFile('./lex-js/public/startPage/info.txt');
